...
namespace PigeonCoopToolkit.Navmesh2D.Editor
{
    ...
    {
        public NavMesh2D.NavmeshGenerationInformation GenerationInformation;
        ...
        private NavMesh2D _sceneNavmesh;
        ...
        [MenuItem("Window/Pigeon Coop Toolkit/Navmesh 3D Generator", false, 1)]
        static void Init() 
        {
        ...
        void OnGUI()
        {
            ...
            int floatAccuracy = EditorGUILayout.IntSlider("Float Percision:",
            ...
            if (jt != GenerationInformation.JoinType)
            {
                EditorPrefs.SetInt("PCTK_NM2DG_JoinType", (int)jt);
                GenerationInformation.JoinType = jt;
            }
            ...
            if(_sceneNavmesh != null)
            {
                ...
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.LabelField("Float Percision: ", EditorStyles.miniLabel);
        ...
        void OnSceneGUI(SceneView view)
        {
            ...
            if (_showNavmesh && _sceneNavmesh && _sceneNavmesh.NavMesh && Event.current.type == EventType.Repaint)
            {
                ...
                foreach (NavMesh2D.MeshContour contour in _sceneNavmesh.Contours)
                {
            ...
            if (_showQuadTree && _sceneNavmesh != null && _sceneNavmesh.NodeQuadTree != null && _sceneNavmesh.NodeQuadTree.QuadTreeNode != null && Event.current.type == EventType.Repaint)
            {
                DrawQuadTreeNodeRecursive(_sceneNavmesh.NodeQuadTree.QuadTreeNode);
            }
        ...
        void RefreshReferences()
        {
            ...
            GenerationInformation = new NavMesh2D.NavmeshGenerationInformation
            {
                ...
                JoinType = (JoinType)EditorPrefs.GetInt("PCTK_NM2DG_JoinType", (int)JoinType.jtSquare),
            ...
            NavMesh2D[] sceneNavMeshes = FindObjectsOfType<NavMesh2D>();
            ...
            else if (sceneNavMeshes.Length == 1)
            {
                ...
                _sceneNavmesh.gameObject.hideFlags = HideFlags.HideInHierarchy;

                if (VersionInformation().Match(_sceneNavmesh.Version) == false)
                {
        ...
        void RunGeneration()
        {
            ...
            NavMesh2D tempNavMesh = tempNavMeshObject.AddComponent<NavMesh2D>();
            ...
            try
            {
                ...
                if(EditorUtility.DisplayCancelableProgressBar("Generating NavMesh2D", "Generating node network", 0.5f))
                    throw new System.OperationCanceledException("User canceled.");
                tempNavMesh.NavNodes = GenerateNodeNetwork(triangulatedPolygons, tempNavMesh.transform);
                EditorUtility.DisplayProgressBar("Generating NavMesh2D", "Finishing up, may take a minute or two", 1f);

                tempNavMesh.NodeQuadTree = tempNavMesh.gameObject.AddComponent<QuadTree>();

                tempNavMesh.NodeQuadTree.SetObjects(tempNavMesh.NavNodes.Select(navNode => new QuadTree.PositionObjectPair
                                                                                               {
                                                                                                   Obj = navNode, Position = navNode.transform.position
                                                                                               }).ToArray());

                tempNavMesh.NodeQuadTree.GenerateQuadTree(GenerationInformation.ObstructionColliderLayer,6);

                tempNavMesh.Navigator = tempNavMesh.gameObject.AddComponent<Navigator>();
                tempNavMesh.Navigator.ObstructionLayer = GenerationInformation.ObstructionColliderLayer;
                tempNavMesh.Navigator.SetNavNodes(tempNavMesh.NavNodes.Select(a => (NavNodeMinor) a).ToArray());
                tempNavMesh.Navigator.NodesQuadTree = tempNavMesh.NodeQuadTree;
                ...
                EditorUtility.SetDirty(tempNavMesh.NodeQuadTree);
                EditorUtility.SetDirty(tempNavMesh.Navigator);
        ...
        private void CleanNavMesh(NavMesh2D navMesh)
        {
            if (navMesh.NavMesh != null)
                DestroyImmediate(navMesh.NavMesh);

            if (navMesh.NavNodes != null && navMesh.NavNodes.Length != 0)
            {
                for (int i = navMesh.NavNodes.Length - 1; i >= 0; i--)
                {
                    NavNode n = navMesh.NavNodes[i];
                    DestroyImmediate(n.gameObject);
                }
            }

            if(navMesh.NodeQuadTree != null)
            {
                DestroyImmediate(navMesh.NodeQuadTree);
            }
            navMesh.NavNodes = null;
        ...
        private void DestroySceneNavMesh()
        {
            if (_sceneNavmesh)
            {
                ...
                GameObject.DestroyImmediate(_sceneNavmesh.gameObject);
            }
        }
        ...
        private PolyTreeEdgesRetained PrepairPolyTree(Collider2D[] floorColliders, Collider2D[] wallColliders)
        {
            ...
            if (floorColliders.Length > 0)
            {
                ...
                {
                    tempCo.AddPath(intPoints, GenerationInformation.JoinType, EndType.etClosedPolygon);
        ...
        private NavMesh2D.MeshContour[] ExtractMeshContour(PolyTreeEdgesRetained ptcr)
        {
            List<NavMesh2D.MeshContour> result = new List<NavMesh2D.MeshContour>();
            ...
            foreach (PolyNode meshContour in ptcr.Tree.Childs)
            {
                ...
                result.Add(new NavMesh2D.MeshContour { Points = extractedPoints.ToArray() });
        ...
        private NavNodeMinor[] GenerateNodeNetwork(List<Polygon> triangulatedPolygon, Transform parent)
        {
            List<NavNodeMinor> allGeneratedNodes = new List<NavNodeMinor>();
            ...
            foreach (Polygon navmesh in triangulatedPolygon)
            {
                ...
                Dictionary<uint, NavNodeMinor> navNodeToVertexCode = new Dictionary<uint, NavNodeMinor>();
                ...
                foreach (DelaunayTriangle t in navmesh.Triangles)
                {
                    foreach (TriangulationPoint p in t.Points)
                    {
                        ...
                        GameObject newNode = new GameObject("Node : " + p.VertexCode);
                        newNode.transform.parent = parent;
                        newNode.transform.position = new Vector3((float)p.X, (float)p.Y, 0);
                        navNodeToVertexCode.Add(p.VertexCode, newNode.AddComponent<NavNodeMinor>());
                    }
                    ...
                    foreach (TriangulationPoint p in t.Points)
                    {
                        foreach (TriangulationPoint pp in t.Points)
                        {
                            if (pp.VertexCode == p.VertexCode)
                                continue;
                            navNodeToVertexCode[p.VertexCode].ConnectTo(navNodeToVertexCode[pp.VertexCode], NavNodeConnection.NavConnectionType.Standard);
                        }
                        if (processedVertices.Contains(p.VertexCode))
                            continue;
                        ...
                        foreach (FixedArray3<TriangulationPoint> point in connectedTriangles)
                        {
                            ...
                            {
                                navNodeToVertexCode[p.VertexCode].ConnectTo(navNodeToVertexCode[triangulationPoint.VertexCode], NavNodeConnection.NavConnectionType.Standard);
                            }
                        }
            ...
            allGeneratedNodes.ForEach(a => a.gameObject.hideFlags = HideFlags.HideInHierarchy);
        ...
        private void DrawQuadTreeNodeRecursive(QuadTreeNode n)
        {
            if(n == null)
                return;

            Handles.color = NavHelper.NavMeshBaseSelectedColor*new Color(1, 1, 1, 0.25f);
            ...
            {
                foreach (QuadTreeNode childNode in n.ChildNodes)
                {
        ...
        {
            return new VersionInformation("NavMesh2D", 1, 4, 6);
        }
